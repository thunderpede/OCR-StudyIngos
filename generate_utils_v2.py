...
def generate_random_image(
    fonts, texts, colors, backgrounds, opacities, sizes,
    max_text_images=5,
    final_size=(512, 512)
):
    # 1) Фон
    ...
    # 2) Сколько кусочков текста делаем
    text_pieces_count = random.randint(1, max_text_images)

    text_images = []
    text_heatmaps = []

    # 3) Генерируем фрагменты
    for i in range(text_pieces_count):
        # Берём случайный шрифт, текст, цвет, т.д.
        ...
        piece_img_np = np.array(piece_img)  # (h, w, 4) RGBA
        hmap = np.stack([char_mask, aff_mask], axis=-1)  # (h, w, 2)
        ...
        if text_pieces_count == 1:
            ...
            # Несколько фрагментов – делаем поменьше
    ...
    for piece_img, piece_hmap in zip(text_images, text_heatmaps):
        ...
        # Если кусок вообще больше фона — можно либо уменьшить ещё сильнее, либо пропустить
        if pw > final_w or ph > final_h:
            # попробуем уменьшить его насильно до fit
            forced_scale_factor = min(final_w / float(pw), final_h / float(ph)) * 0.95
            if forced_scale_factor <= 0.1:
                # всё равно слишком большой, пропустим
                continue
            piece_img, piece_hmap = scale_piece(piece_img, piece_hmap, forced_scale_factor)
            ph, pw = piece_img.shape[:2]

        # Пытаемся найти (x, y), где нет пересечения
        for _ in range(30):
            ...
            # Не нашли место за 30 попыток — пропустим этот кусок
            pass
...
def generate_text_image(
    ...
    """
    Функция из вашего кода — без изменений (или с небольшими правками).
    Возвращает кусок изображения (RGBA) и 2D-маски символов и affinity.
    """
    ...
    # КрОпнем итог по факту напечатанного
